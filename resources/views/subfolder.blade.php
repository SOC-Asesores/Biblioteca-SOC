...
@section('content')
        ...
        <div class="container mb-4 position-relative">
            @if(Auth::user()->id_bank == 0)
                <ul style="list-style: none; padding-left: 0;" class="mt-4 mb-4">
                    <li class="ml-0 d-inline-block"><a href="{{ url('/') }}">Inicio</a></li>
                    @php
                        $count_migas = 0;
                    @endphp
                    @foreach ($migas as $miga)
                        <li class="ml-0 d-inline-block"><a href="{{ $miga }}"> / {{ $migas_name[$count_migas] }}</a></li>
                        @php
                            $count_migas++;
                        @endphp
            ...
            <div class="row justify-content-between" style="background: #f3f3f3;">
                <div class="col-12">
                    <div class="delete-multiple-btn" style="display:none;">
                        ...
                        @if(Auth::user()->id_bank == 0 && Auth::user()->type == 0)
                        
                            <a href="" class="mr-4 delete-multiple" data-toggle="modal" data-target="#exampleModalDelete2" style="color: rgb(255, 113, 80)!important"><i class="fa-solid fa-trash-can"></i> Eliminar seleccionados</a>
                        @endif 
                        
                        <a href="" id="cancel_selection" style="border: 0px;     font-size: .7rem;"><i class="fa fa-times" style="color: red"></i>  Cancelar selección</a>
                    </div>
                </div>
                    ...
                    @if (isset($sub_folder->id_folder))
                        @if ($id_folder === $sub_folder->id_folder)
                            <div class="col-md-2">
                                ...
                                @php 
                                    $folders_and_files = FALSE;
                                    if (isset($archives) && !empty($archives)){
                                        foreach($archives as $key => $archive){
                                            if (isset($archive->id_folder) && $id_folder === $archive->id_folder){
                                                $folders_and_files = TRUE;
                                            }
                                        }
                                    }
                                @endphp

                                @if($folders_and_files == TRUE)
                                <a href="{{url('/')}}/folder/{{$sub_folder->url}}" id="folder_{{$sub_folder->id}}">
                                    <div class="content text-center relative">
                                        <div class="carpeta-icono {{ $sub_folder->hide == 0 ? '' : 'mb-4' }}">
                                            <span></span>
                                            <span></span>
                                        ...
                                        <div class="menu-more grid-view-name" id="menu-more-{{$sub_folder->id}}">
                                            <p class="{{ $sub_folder->hide == 0 ? '' : 'd-none' }}">
                                                {{ strlen($sub_folder->name) > 35 ? mb_substr($sub_folder->name, 0, 35).'...' : $sub_folder->name}}
                                            </p>
                                        </div>
                                ...
                                @if (Auth::check() && Auth::user()->type === 0)
                                    ...
                                    <div class="menu-more-items small-more-items d-none" id="more_{{$sub_folder->id}}">  
                                        ...
                                        @if(
                                            (Auth::user()->id_bank == 0 && Auth::user()->id_bank == 6) ||
                                            in_array(Auth::user()->id_bank,[1,2,6])
                                        )
                        ...
                        @if ($id_folder === $archives->id_folder)
                            <div class="col-md-2">  
                                @if (Auth::check())   
                                    ...
                                    @endif
                                @endif
                                <a href="{{url('img')}}/archivos/{{$archives->img}}" class="counter_file" data-id="{{$archives->id}}" id="files_{{$archives->id}}" target="_blank">
                                    <div class="content text-center">
                                        ...
                                        <div class="menu-more grid-view-name" id="menu-more-{{$archives->id}}">
                                            <p class="{{ $archives->hide == 0 ? '' : 'd-none' }}">{{ strlen($archives->img) > 35 ? mb_substr($archives->img, 0, 35).'...' : $archives->img }}</p>
    ...
    <style>body{padding-bottom:120px}.more-name{padding-right:1.65em}.more-name-btn-submit{right:0;bottom:.65em;}</style>
@endsection
